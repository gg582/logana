...
double logana_knee_detect_eps(const logana_feature_matrix_t *matrix,
    ...
    if (!kdists) return 1.0;

    for (size_t i = 0; i < rows; ++i) {
        double *dists = malloc(rows * sizeof(double));
        if (!dists) { free(kdists); return 1.0; }
    ...
    /* Sort k-distances descending for knee plot */
    qsort(kdists, rows, sizeof(double), kd_compare_asc);

    /* Discrete 2nd derivative on the sorted curve */
    ...
    /* Guard against degenerate flat line or zero eps */
