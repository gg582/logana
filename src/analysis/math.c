...
/* -------------------------------------------------------------------------- */
/* Absolute clamp for numeric values.  This is a physical safeguard against   */
/* IEEE-754 infinities propagating into downstream linear algebra, NOT a      */
/* geometric / scale-dependent threshold.                                     */
#define LOGANA_NUMERIC_ABS_MAX 9.223372036854776e18
...
static int logana_safe_strtod(const char *str, double *out, bool *out_valid, bool clamp_negative) {
    ...
    if (fabs(value) > LOGANA_NUMERIC_ABS_MAX) {
        if (out_valid) *out_valid = true;
        *out = (value > 0.0) ? LOGANA_NUMERIC_ABS_MAX : -LOGANA_NUMERIC_ABS_MAX;
        return 1;
    }
...
/* -------------------------------------------------------------------------- */
static double logana_get_min_stddev_floor(const logana_analysis_summary_t *summary, size_t dim) {
...
static double logana_get_winsorize_multiplier(const logana_analysis_summary_t *summary) {
    if (!summary) return 5.0;
    return summary->cluster_options.has_winsorize_sigma_multiplier
               ? summary->cluster_options.winsorize_sigma_multiplier
               : 5.0;
}

static double logana_get_outlier_zscore_threshold(const logana_analysis_summary_t *summary) {
    if (!summary) return 9.0;
    return summary->cluster_options.has_outlier_zscore_threshold
               ? summary->cluster_options.outlier_zscore_threshold
               : 9.0;
}
...
double logana_distance_euclidean_sq(const double *a, const double *b,
    ...
    (void)summary; /* unused, but part of vtable contract */
    if (!a || !b || dims == 0) return HUGE_VAL;
    double total = 0.0;
...
double logana_distance_zscore_sq(const double *a, const double *b,
    ...
    for (size_t i = 0; i < dims; ++i) {
        ...
        double sigma = 1.0;
        if (summary) {
            double floor = logana_get_min_stddev_floor(summary, i);
            sigma = summary->stddev[i] > floor ? summary->stddev[i] : floor;
        }
...
static void *logana_memdup_bytes(const void *src, size_t bytes) {
    ...
    if (!dst) return NULL;
    if (bytes > 0) memcpy(dst, src, bytes);
    return dst;
}

static void logana_free_matrix_buffers(double *values, uint64_t *timestamps,
                                       uint8_t *valid_mask, uint64_t *categories,
                                       uint8_t *formats) {
    free(values);
    free(timestamps);
    free(valid_mask);
...
static bool logana_grow_matrix_buffers(double **values,
                                       ...
                                       size_t new_cap,
                                       size_t dims) {
    if (!values || !timestamps || !valid_mask || !categories || !formats) return false;
    size_t old_cells = 0;
    size_t new_cells = 0;
    ...
    double *new_values = calloc(new_cells, sizeof(double));
    uint64_t *new_timestamps = calloc(new_cap, sizeof(uint64_t));
    uint8_t *new_valid_mask = calloc(new_cells, sizeof(uint8_t));
    ...
    if (old_cells > 0 && *values) memcpy(new_values, *values, old_cells * sizeof(double));
    if (old_cap > 0 && *timestamps) memcpy(new_timestamps, *timestamps, old_cap * sizeof(uint64_t));
    if (old_cells > 0 && *valid_mask) memcpy(new_valid_mask, *valid_mask, old_cells * sizeof(uint8_t));
    if (old_cap > 0 && *categories) memcpy(new_categories, *categories, old_cap * sizeof(uint64_t));
    if (old_cap > 0 && *formats) memcpy(new_formats, *formats, old_cap * sizeof(uint8_t));

    logana_free_matrix_buffers(*values, *timestamps, *valid_mask, *categories, *formats);
...
size_t logana_parse_matrix(logana_engine_t *engine, logana_job_t *job) {
    if (!engine || !job || !job->payload) return 0;
    ...
    double *values = calloc(cell_capacity, sizeof(double));
    uint64_t *timestamps = calloc(row_capacity, sizeof(uint64_t));
    uint8_t *valid_mask = calloc(cell_capacity, sizeof(uint8_t));
    ...
    while (cursor && *cursor && rows < engine->config.max_rows_per_analysis) {
        ...
        if (*cursor) {
            ...
            if (valid_mask) memset(valid_mask + rows * dims, 1, dims * sizeof(uint8_t));
            ...
            /* Strip timestamp from line to avoid double-parsing */
            for (size_t t = 0; t < engine->config.timestamp_key_count; ++t) {
                const char *found = engine->config.case_sensitive
                    ? strstr(cursor, engine->config.timestamp_keys[t])
                    : logana_find_key_ci(cursor, engine->config.timestamp_keys[t]);
                if (found) {
                    const char *colon = strchr(found, ':');
                    if (colon) {
                        char *p = scratch + (colon + 1 - scratch);
                        while (*p && *p != ',' && *p != '}') { *p = ' '; ++p; }
                        break;
                    }
                }
...
void logana_compute_summary(logana_job_t *job) {
    ...
    for (size_t r = 0; r < rows; ++r) {
        bool primary_valid = (!job->matrix.valid_mask || job->matrix.valid_mask[r * dims]);
        double primary = job->matrix.values[r * dims];
        if (primary_valid && isfinite(primary) && fabs(primary) <= LOGANA_NUMERIC_ABS_MAX) {
        ...
        double dt = t - t_mean;
        if (primary_valid && isfinite(primary) && fabs(primary) <= LOGANA_NUMERIC_ABS_MAX) { slope_num += dt * primary; slope_den += dt * dt; }
        for (size_t d = 0; d < dims; ++d) {
            if (job->matrix.valid_mask && !job->matrix.valid_mask[r * dims + d]) continue;
            double value = job->matrix.values[r * dims + d];
            if (!isfinite(value) || fabs(value) > LOGANA_NUMERIC_ABS_MAX) continue;
    ...
    double winsor_mult = logana_get_winsorize_multiplier(&job->summary);
    ...
    for (size_t d = 0; d < dims; ++d) {
        ...
        for (size_t r = 0; r < rows; ++r) {
            ...
            if (!isfinite(v) || fabs(v) > LOGANA_NUMERIC_ABS_MAX) continue;
            vals[idx++] = v;
        }
        qsort(vals, n, sizeof(double), logana_compare_double);
        double median = vals[n / 2];
        if ((n % 2) == 0 && n > 1) median = (vals[n / 2 - 1] + vals[n / 2]) * 0.5;
        for (size_t i = 0; i < n; ++i) vals[i] = fabs(vals[i] - median);
        qsort(vals, n, sizeof(double), logana_compare_double);
        double mad = vals[n / 2];
        if ((n % 2) == 0 && n > 1) mad = (vals[n / 2 - 1] + vals[n / 2]) * 0.5;

        double min_floor = logana_get_min_stddev_floor(&job->summary, d);
        double mad_sigma = mad > min_floor ? mad * 1.4826 : min_floor;
        double lower = median - winsor_mult * mad_sigma;
        double upper = median + winsor_mult * mad_sigma;
        double wsum = 0.0;
        for (size_t r = 0; r < rows; ++r) {
            if (job->matrix.valid_mask && !job->matrix.valid_mask[r * dims + d]) continue;
            double v = job->matrix.values[r * dims + d];
    ...
    double outlier_threshold = logana_get_outlier_zscore_threshold(&job->summary);
    size_t outliers = 0;
    for (size_t r = 0; r < rows; ++r) {
        double score = 0.0;
        size_t valid_d = 0;
        for (size_t d = 0; d < dims; ++d) {
            if (job->matrix.valid_mask && !job->matrix.valid_mask[r * dims + d]) continue;
            double floor = logana_get_min_stddev_floor(&job->summary, d);
            double sigma = job->summary.stddev[d] > floor ? job->summary.stddev[d] : floor;
            double z = (job->matrix.values[r * dims + d] - job->summary.mean[d]) / sigma;
            score += z * z;
            ++valid_d;
        }
        if (valid_d > 0 && score > outlier_threshold) ++outliers;
    }
