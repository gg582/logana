...
static double kmeans_inertia(const logana_feature_matrix_t *matrix, size_t k,
    ...
    if (!matrix || !matrix->values || !centers || !dist_fn) return HUGE_VAL;
...
static void kmeans_pp_init(const logana_feature_matrix_t *matrix, size_t k, uint64_t seed,
                           ...
                           const logana_analysis_summary_t *summary) {
    if (!matrix || !matrix->values || !centers || !dist_fn) return;
    size_t rows = matrix->row_count;
    size_t dims = matrix->dimensions;
    if (!rows || !k) return;
    uint64_t rng = seed;
...
static size_t run_kmeans(const logana_feature_matrix_t *matrix, size_t k, size_t seed_offset,
                         ...
                         const logana_analysis_summary_t *summary,
                         int *out_labels) {
    if (!matrix || !matrix->values || !out_labels || !dist_fn) return 0;
    size_t rows = matrix->row_count;
    size_t dims = matrix->dimensions;
    if (!rows) return 0;
    if (k > rows) k = rows;
    if (k == 0) k = 1;
    if (summary && summary->cluster_options.has_kmeans_max_k && summary->cluster_options.kmeans_max_k > 0) {
        if (k > summary->cluster_options.kmeans_max_k) k = summary->cluster_options.kmeans_max_k;
    } else {
        if (k > 8) k = 8;
    }

    double *centers = calloc(k * dims, sizeof(double));
    double *best_centers = calloc(k * dims, sizeof(double));
    int *tmp_labels = calloc(rows, sizeof(int));
    if (!centers || !best_centers || !tmp_labels) {
        free(centers); free(best_centers); free(tmp_labels);
        return 0;
    }

    size_t n_init = (rows > 5000) ? 3 : 5;
    if (summary && summary->cluster_options.has_kmeans_n_init && summary->cluster_options.kmeans_n_init > 0) {
        n_init = summary->cluster_options.kmeans_n_init;
    }
    double best_inertia = DBL_MAX;

    double *accum = calloc(k * dims, sizeof(double));
    size_t *counts = calloc(k, sizeof(size_t));
    if (!accum || !counts) {
        free(accum); free(counts);
        free(centers); free(best_centers); free(tmp_labels);
        return 0;
    }
    ...
    for (size_t trial = 0; trial < n_init; ++trial) {
        ...
        size_t max_iter = 30;
        if (summary && summary->cluster_options.has_kmeans_iterations && summary->cluster_options.kmeans_iterations > 0) {
            max_iter = summary->cluster_options.kmeans_iterations;
        }
        for (size_t iter = 0; iter < max_iter; ++iter) {
            memset(accum, 0, k * dims * sizeof(double));
        ...
        if (inertia < best_inertia) {
            ...
            memcpy(best_centers, centers, k * dims * sizeof(double));
            memcpy(out_labels, tmp_labels, rows * sizeof(int));
        }
    }

    memcpy(centers, best_centers, k * dims * sizeof(double));
    for (size_t r = 0; r < rows; ++r) {
        double best_dist = DBL_MAX;
        int best = 0;
        for (size_t c = 0; c < k; ++c) {
...
#define LOGANA_KMEANS_AUTO_CAP_ROWS 100000
...
static size_t run_kmeans_auto(const logana_feature_matrix_t *matrix,
    ...
    if (rows > LOGANA_KMEANS_AUTO_CAP_ROWS) {
